...
export default function ArtilheiroFDK({ listOfPlayers, listOfMatches }) {
  ...
  const getPlayers = () => {
    ...
    listOfMatches.forEach(partida => {
      partida.jogadoresTime1.forEach(pl => {
        ...
        dataArray[pos].media = (dataArray[pos].gol / dataArray[pos].partida)
          .toFixed(2)
          .replace('.', ',')
      })
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {players.map(rank => (
          <View style={styles.dataContainer} key={rank.jogador}>
            ...
            <Text style={styles.golTitle}>{rank.gol}</Text>
